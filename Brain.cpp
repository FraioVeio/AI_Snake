...
void Brain::compute(float *inputs, float *outputs) {
    ...
    float (*bi)[hiddenLayers][neuronsPerLayer] = (float (*)[hiddenLayers][neuronsPerLayer]) biases;

    /* Compute first hidden layer */
    memset(neuronValuesPrev, 0, neuronsPerLayer*sizeof(float));
    for(int i=0;i<inputSize;i++) {
        for(int h=0;h<neuronsPerLayer;h++) {
            neuronValuesPrev[h] += inputs[i] * (*iw)[i][h]/inputSize;
        }
    }
    for(int h=0;h<neuronsPerLayer;h++) {
    ...
    for(int l=0;l<hiddenLayers-1;l++) {
        ...
        for(int i1=0;i1<neuronsPerLayer;i1++) {
            for(int i2=0;i2<neuronsPerLayer;i2++) {
                neuronValuesCurr[i1] += neuronValuesPrev[i2] * (*lw)[l][i2][i1];
            }
            neuronValuesCurr[i1] += (*bi)[l][i1];
        }
...
genome_t Brain::getGenome() {
    ...
    offset += (hiddenLayers-1) * neuronsPerLayer * neuronsPerLayer;

    memcpy(&genome[offset], biases, hiddenLayers*neuronsPerLayer*sizeof(float));
...
void Brain::setGenome(genome_t genomes) {
    ...
    memcpy(biases, &genome[offset], hiddenLayers*neuronsPerLayer*sizeof(float));
}

int Brain::getGenomeArraySize() {
    return (inputSize*neuronsPerLayer + outputSize*neuronsPerLayer + (hiddenLayers-1) * neuronsPerLayer * neuronsPerLayer + hiddenLayers * neuronsPerLayer);
}
...
Brain::Brain(int inputSize, int outputSize, int hiddenLayers, int neuronsPerLayer) {
    ...
    biases = (float*) calloc(hiddenLayers * neuronsPerLayer, sizeof(float));
}
